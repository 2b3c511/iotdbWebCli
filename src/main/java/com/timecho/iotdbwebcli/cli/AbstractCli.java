package com.timecho.iotdbwebcli.cli;
...
import com.timecho.iotdbwebcli.exception.ArgsErrorException;
import com.timecho.iotdbwebcli.util.IoTDBPrinter;
...
public abstract class AbstractCli {
  ...
  static String checkRequiredArg(
    ...
    if (str == null) {
      if (isRequired) {
        ...
        IoTDBPrinter.println(msg);
        IoTDBPrinter.println("Use -help for more information");
  ...
  private static int setFetchSize(String specialCmd, String cmd) {
    ...
    try {
      ...
      IoTDBPrinter.println(String.format("Fetch size format error, %s", e.getMessage()));
      return CODE_ERROR;
    }
    IoTDBPrinter.println("Fetch size has set to " + values[1].trim());
    return CODE_OK;
  }
  ...
  static void echoStarting() {
    IoTDBPrinter.println("---------------------");
    IoTDBPrinter.println("Starting IoTDB Cli");
    IoTDBPrinter.println("---------------------");
  }
  ...
  static OperationResult handleInputCmd(String cmd, IoTDBConnection connection) {
    ...
    if (specialCmd.startsWith(SHOW_TIMESTAMP_DISPLAY)) {
      IoTDBPrinter.println("Current time format: " + timeFormat);
      return OperationResult.CONTINUE_OPER;
    }
    if (specialCmd.startsWith(SHOW_FETCH_SIZE)) {
      IoTDBPrinter.println("Current fetch size: " + fetchSize);
      return OperationResult.CONTINUE_OPER;
    }

    // TODO: webCli doesn't support import yet.
    //        if (specialCmd.startsWith(IMPORT_CMD)) {
    //            lastProcessStatus = IoTDBPrinter.importCmd(specialCmd, cmd, connection);
    //            return OperationResult.CONTINUE_OPER;
    //        }
  ...
  private static void showHelp() {
    IoTDBPrinter.println("    <your-sql>\t\t\t execute your sql statment");
    IoTDBPrinter.println(
    ...
    IoTDBPrinter.println(String.format("    %s\t show time display type", SHOW_TIMESTAMP_DISPLAY));
    IoTDBPrinter.println(String.format("    %s=xxx\t\t eg. +08:00, Asia/Shanghai.", SET_TIME_ZONE));
    IoTDBPrinter.println(String.format("    %s\t\t show cli time zone", SHOW_TIMEZONE));
    IoTDBPrinter.println(
        String.format(
            "    %s=xxx\t\t set fetch size when querying data from server.", SET_FETCH_SIZE));
    IoTDBPrinter.println(String.format("    %s\t\t show fetch size", SHOW_FETCH_SIZE));
  }
  ...
  private static int setTimestampDisplay(String specialCmd, String cmd) {
    ...
    try {
      ...
      IoTDBPrinter.println(String.format("time display format error, %s", e.getMessage()));
      return CODE_ERROR;
    }
    IoTDBPrinter.println("Time display type has set to " + cmd.split("=")[1].trim());
    return CODE_OK;
  }
  ...
  private static int setTimeZone(String specialCmd, String cmd, IoTDBConnection connection) {
    ...
    try {
      ...
      IoTDBPrinter.println(String.format("Time zone format error: %s", e.getMessage()));
      return CODE_ERROR;
    }
    IoTDBPrinter.println("Time zone has set to " + values[1].trim());
    return CODE_OK;
  }

  private static int showTimeZone(IoTDBConnection connection) {
    try {
      IoTDBPrinter.println("Current time zone: " + connection.getTimeZone());
    } catch (Exception e) {
      IoTDBPrinter.println("Cannot get time zone from server side because: " + e.getMessage());
      return CODE_ERROR;
    }
    return CODE_OK;
  }

  // TODO:webCli doesn't support import now.
  ...
  //            return CODE_ERROR;
  //        }
  //        IoTDBPrinter.println(cmd.split(" ")[1]);
  //        try {
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      if (hasResultSet) {
        ...
        try (ResultSet resultSet = statement.getResultSet()) {
          ...
          IoTDBPrinter.println(String.format("It costs %.3fs", costTime / 1000.0));
          while (!isReachEnd) {
            if (continuePrint) {
        ...
        IoTDBPrinter.println("Msg: " + SUCCESS_MESSAGE);
      }
    } catch (Exception e) {
      IoTDBPrinter.println("Msg: " + e.getMessage());
      executeStatus = CODE_ERROR;
    } finally {
      resetArgs();
    }
  ...
  private static List<List<String>> cacheResult(
    ...
    if (resultSet instanceof IoTDBJDBCResultSet) {
      for (int i = 1; i <= columnCount; i++) {
        ...
        lists.add(list);
        int count = IoTDBPrinter.computeHANCount(columnLabel);
      ...
      while (j < maxPrintRowCount && !isReachEnd) {
        for (int i = 1; i <= columnCount; i++) {
          ...
          int count = IoTDBPrinter.computeHANCount(tmp);
  ...
  private static void output(List<List<String>> lists, List<Integer> maxSizeList) {
    IoTDBPrinter.printBlockLine(maxSizeList);
    IoTDBPrinter.printRow(lists, 0, maxSizeList);
    IoTDBPrinter.printBlockLine(maxSizeList);
    for (int i = 1; i < lists.get(0).size(); i++) {
      IoTDBPrinter.printRow(lists, i, maxSizeList);
    }
    IoTDBPrinter.printBlockLine(maxSizeList);
    if (isReachEnd) {
      lineCount += lists.get(0).size() - 1;
      IoTDBPrinter.printCount(lineCount);
    } else {
  ...
  private static void outputTracingInfo(List<List<String>> lists, List<Integer> maxSizeList) {
    IoTDBPrinter.println();
    IoTDBPrinter.println("Tracing Activties:");
    IoTDBPrinter.printBlockLine(maxSizeList);
    IoTDBPrinter.printRow(lists, 0, maxSizeList);
    IoTDBPrinter.printBlockLine(maxSizeList);
    for (int i = 1; i < lists.get(0).size(); i++) {
